...
void computeNormal(int i, int j, std::vector<float> &normal) {
    ...
    std::vector<float> vector1 = {p2x - p1x, p2y - p1y, p2z - p1z};
    std::vector<float> vector2 = {p4x - p3x, p4y - p3y, p4z - p3z};

    float res[3];
    cross(vector1.data(), vector2.data(), res);
    normalize(res);
    ...
    // Aqui estamos a ignorar os vértices das bordas, mas nao deviamos
}
